...
public class CaveGenerator
{
    ...
    protected float randomFillPercentBetweenWallAndGrass;
    protected float randomFillPercentBetweenGrassAndBushes;
    ...
    public CaveGenerator(float randomFillPercentBetweenWallAndGrass = 0.333f,
        float randomFillPercentBetweenGrassAndBushes = 0.633f, int gridSize = 100)
    {
        this.randomFillPercentBetweenWallAndGrass = randomFillPercentBetweenWallAndGrass;
        this.randomFillPercentBetweenGrassAndBushes = randomFillPercentBetweenGrassAndBushes;
        ...
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                else
                {
                    ...
                    if (random.NextDouble() <= randomFillPercentBetweenWallAndGrass)
                    {
                        bufferOld[x, y] = 1;
                    }
                    else if (random.NextDouble() > randomFillPercentBetweenWallAndGrass &&
                             random.NextDouble() <= randomFillPercentBetweenGrassAndBushes)
                    {
                        bufferOld[x, y] = 0;
                    }
                    else
    ...
    public void SmoothMap()
    {
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                int surroundingSea = GetSurroundingSeaCount(x, y);

                //Use some smoothing rules to generate caves
                if (surroundingWalls > 3 && surroundingWalls > surroundingSea)
                {
                    bufferNew[x, y] = 1;
                }
                else if (surroundingSea > 3 && surroundingSea > surroundingWalls)
                {
                    bufferNew[x, y] = 2;
                }
                // else if (surroundingWalls == 3||surroundingSea==3) {
    ...
    //Given a cell, how many of the 8 surrounding cells are walls?
    private int GetSurroundingSeaCount(int cellX, int cellY)
    {
        int seaCounter = 0;
        for (int neighborX = cellX - 1; neighborX <= cellX + 1; neighborX++)
        {
